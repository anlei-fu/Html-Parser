...
using Fal.Net;
using Fal.StateMachine;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace ConsoleApp1.StateMachine
{
    /************************************
     * html parser
    ...
    public class HtmlParser
    {
        ...
        /// record errors
        /// </summary>
        public List<PaserException> Errors = new List<PaserException>();
        /// <summary>
                ...
                switch (_token)
                {
                    case '<':
                        ...
                        else
                        {
                             ...
                             * 生成新节点
                             * ***********************/
                            _temp = new Element() { StartOrEnd = StartOrEnd.START, Positionstart = _INDEX_ - 1 };
                    ...
                    case '>':
                        if (_temp != null)
                        {
                             ...
                             * 记录结束位置
                             * ****************/
                            _temp.Positionend = _INDEX_ - 1;
                            /*****************
                             * 处理未知元素问题
                             * **********************/
                            if (_temp.ElementType == ElementType.unknow)
                            {
                            ...
                            * 处理脚本问题
                            * *********************/
                            if (_temp.StartOrEnd == StartOrEnd.START)
                                handScript();
                            /*************
                             * 添加进栈
                             * ********************/
                    ...
                    case '/':
                        ...
                        if (!_isStartOrEndHasBeenSet)
                            _temp.StartOrEnd = StartOrEnd.END;
                        /*******************
                         * two '/'
                         * ****************************/
                        else
                    ...
                    default:
                        ...
                        if (_isWorkingInElement)
                        {
                            ...
                            if (!_isElementNameHasBeenSet)
                            {
                                ...
                                _temp.ElementType = SemiElement.GetType(_temp.Name);
                                _temp.SingleOrDouble = SemiElement.GetSingleOrDouble(_temp.ElementType);
                             ...
                             * *****************************/
                                ...
                                _item = new AttributeItem() { Key = _sb.ToString() };
                            /*******************
                             * name=ch 这么奇葩形式 
                             * *******************/
                            else
                        ...
                        else
                        {
                             ...
                             * 单标签不能包含innertext
                             * **********************************/
                            else if (_stack.Peek().SingleOrDouble == SingleOrDouble.SINGLE)
                                error(_sb.ToString());
                            /**********************
                             * 正确
                             * *****************/
                            ...
                            if (_INDEX_ + 2 != _str.Length)
                            {
                                if (_str[_INDEX_ + 1] == '-')
                                {
                                    if (_str[_INDEX_ + 2] == '>')
                                    {
                                        ...
                                        _temp.SingleOrDouble = SingleOrDouble.SINGLE;
                                        _sb.Clear();
                                        setStack();
                                        reset();
                                        _INDEX_ += 3;
        ...
        private void setStack()
        {
            ...
            if (_ignorDenote)
            {
                if (_temp.ElementType == ElementType.denote)
                    return;
            }
            ...
            {
                if (_temp.SingleOrDouble == SingleOrDouble.BOUBLE)
                {
             ...
             * 准备压入单标签
             * ****************************/
            if (_temp.SingleOrDouble == SingleOrDouble.SINGLE)
            {
                /***************
                 * peek is single
                 * *********************/
                if (peek.SingleOrDouble == SingleOrDouble.SINGLE)
                {
                    peek.Next = _temp;
                    _temp.Previous = peek;
                    _stack.Push(_temp);
            ...
            else
            {
                ...
                if (_temp.StartOrEnd == StartOrEnd.START)
                {
                     ...
                     * peek is double
                     * ************************/
                    if (peek.SingleOrDouble == SingleOrDouble.BOUBLE)
                    {
                        _stack.Push(_temp);
                        peek.Children.Add(_temp);
                        _temp.Father = peek;
                ...
                else
                {
                     ...
                     * ***************************/
                    if (peek.SingleOrDouble == SingleOrDouble.SINGLE)
                        error($"elementype doesn't match {_temp.Positionstart}-{_temp.Positionend}");
                    else
                    {
                        /**********************
                         * 类型匹配出栈
                         * ******************************/
                        if (peek.ElementType == _temp.ElementType)
                        {
                            peek.Positionend = _temp.Positionend;
                            _stack.Pop();
        ...
        private void getRoot()
        {
            ...
            else
            {
                ...
                {
                    var b = _stack.Pop();
                    if (b.SingleOrDouble == SingleOrDouble.BOUBLE)
                        flag = true;
        ...
        {
            Errors.Add(new PaserException(_LINE_, _INDEXOFLINE_, _INDEX_, _Error_Number, _FILE_, description));
            ++_Error_Number;
        }
    }
}
